	...
	public void onInitialize() {
		...
		net.fabricmc.fabric.api.event.player.UseItemCallback.EVENT.register((player, world, hand) -> {
			if (!world.isClient()) {
				...
				if (stack.getItem() == net.minecraft.item.Items.PAPER) {
					net.minecraft.component.type.NbtComponent nbtComponent = stack.getComponents()
					...
					if (nbtComponent != null) {
						...
						if (nbt.contains("EconomyBankNote") && nbt.contains("Value")) {
							...
							player.sendMessage(net.minecraft.text.Text
									.literal("Redeemed bank note for " + EconomyManager.getInstance().format(value))
									.formatted(net.minecraft.util.Formatting.GREEN), true);
							savage.commoneconomy.util.TransactionLogger.log("DEPOSIT", "Bank Note",
									player.getName().getString(), value, "Redeemed Note");
							stack.decrement(1);
		...
		if (EconomyManager.getInstance().getConfig().enableChestShops) {
					...
					for (net.minecraft.util.math.Direction dir : directions) {
						...
						if (shop != null) {
							...
							if (ShopCommands.isInRemoveMode(serverPlayer.getUuid())) {
								...
								boolean isAdmin = savage.commoneconomy.util.PermissionsHelper.check(serverPlayer,
								...
								if (isOwner || isAdmin) {
									...
									serverPlayer.sendMessage(
											net.minecraft.text.Text.literal("§cYou don't own this shop!"), false);
									ShopCommands.exitRemoveMode(serverPlayer.getUuid());
							...
							savage.commoneconomy.shop.ShopInteractionManager.getInstance().addPendingInteraction(
									...
									shop,
									shop.isBuying() // If shop is buying, player is selling (and vice versa? wait logic
													// check)
							);
							...
							serverPlayer.sendMessage(net.minecraft.text.Text
									.literal("§eType the amount you want to " + action + " in chat."), false);
							serverPlayer.sendMessage(
									net.minecraft.text.Text.literal("§eType 'all' to " + action + " everything."),
									false);
			...
			net.fabricmc.fabric.api.message.v1.ServerMessageEvents.ALLOW_CHAT_MESSAGE
					.register((message, sender, params) -> {
						savage.commoneconomy.shop.ShopInteractionManager.PendingInteraction interaction = savage.commoneconomy.shop.ShopInteractionManager
								.getInstance().getPendingInteraction(sender.getUuid());

						if (interaction != null) {
							if (interaction.isExpired()) {
								savage.commoneconomy.shop.ShopInteractionManager.getInstance()
							...
							if (!isAll) {
								try {
									...
									sender.sendMessage(
											net.minecraft.text.Text.literal("§cInvalid amount! Transaction cancelled."),
											false);
									savage.commoneconomy.shop.ShopInteractionManager.getInstance()
											.removePendingInteraction(sender.getUuid());
									return false;
							...
							boolean shopIsBuying = shop.isBuying(); // true = shop buys (player sells), false = shop
																	// sells (player buys)
							...
							if (shopIsBuying) {
								if (isAll) {
									...
									for (int i = 0; i < sender.getInventory().size(); i++) {
										...
										if (net.minecraft.item.ItemStack.areItemsAndComponentsEqual(stack,
												shop.getItem())) {
								...
								if (amount > 0) {
									savage.commoneconomy.shop.ShopTransactionHandler.handleSellTransaction(
											(net.minecraft.server.world.ServerWorld) sender.getEntityWorld(), sender,
											shop, amount);
								} else {
									sender.sendMessage(
											net.minecraft.text.Text.literal("§cYou don't have any items to sell!"),
											false);
								}
							}
							// If shop is selling, player is buying from it
								...
								if (isAll) {
									...
									java.math.BigDecimal balance = EconomyManager.getInstance()
											.getBalance(sender.getUuid());
									...
									if (amount > 2304)
										amount = 2304; // 36 stacks (full inventory)
								}

								if (amount > 0) {
									savage.commoneconomy.shop.ShopTransactionHandler.handleBuyTransaction(
											(net.minecraft.server.world.ServerWorld) sender.getEntityWorld(), sender,
											shop, amount);
								} else {
									sender.sendMessage(net.minecraft.text.Text
											.literal("§cYou cannot afford any items or shop is out of stock!"), false);
								}
							}
						...
						if (state.getBlock() instanceof net.minecraft.block.ChestBlock) {
							if (ShopManager.getInstance().isShopChest(pos)) {
								player.sendMessage(net.minecraft.text.Text.literal(
										"§cYou cannot break shop chests! Remove the shop first with /shop remove."),
										false);
								return false;
							}
						}
							...
							for (net.minecraft.util.math.Direction dir : directions) {
								...
								if (shop != null) {
									...
									if (player instanceof net.minecraft.server.network.ServerPlayerEntity serverPlayerEntity) {
										isAdmin = savage.commoneconomy.util.PermissionsHelper.check(serverPlayerEntity,
												"savscommoneconomy.admin", 2);
									}
									...
									if (!isOwner && !isAdmin) {
										player.sendMessage(
												net.minecraft.text.Text.literal(
														"§cYou cannot break this shop sign! Use /shop remove instead."),
												false);
										return false;
									}
									// If owner/admin breaks sign, remove the shop
									ShopManager.getInstance().removeShop(chestPos);
									player.sendMessage(net.minecraft.text.Text.literal("§eShop removed (sign broken)."),
											false);
									return true;
								}
							}
			...
			// Register server tick end event to clean up orphaned shops (shops without
			// chests)
			net.fabricmc.fabric.api.event.lifecycle.v1.ServerTickEvents.END_SERVER_TICK.register(server -> {
				...
				if (server.getTicks() % 20 == 0) {
					for (net.minecraft.server.world.ServerWorld world : server.getWorlds()) {
						...
						// Note: This is a bit inefficient if there are thousands of shops, but fine for
						// now
						...
						for (savage.commoneconomy.shop.Shop shop : ShopManager.getInstance().getAllShops()) {
							if (worldId.equals(shop.getWorldId())) {
								net.minecraft.util.math.BlockPos signPos = savage.commoneconomy.shop.ShopSignHelper
										.findSignForChest(world, shop.getChestLocation());
								if (signPos != null) {
									// Calculate current stock
									int currentStock = savage.commoneconomy.shop.ShopStockCalculator
											.calculateStock(world, shop);
